...
function validate(payload: any) {
  ...
  if (
    !["Desktop computer", "Laptop", "Mobile phone", "Tablet", "Other"].includes(
  ...
  for (const answer of answers) {
    ...
    if (!q || seen.has(answer.questionId)) {
      errors[answer?.questionId || "answers"] = "This answer is not recognised.";
      continue;
    }
    seen.add(answer.questionId);
    if (
      q.showWhen &&
      answers.find((a: any) => a.questionId === q.showWhen?.questionId)?.textAnswer !==
        q.showWhen.equals
    )
      continue;
    if (
      ...
      errors[q.id] = `Please answer: ${q.text}`;
    if (
      q.type === "rating" &&
      (!Number.isInteger(answer.numericAnswer) ||
        answer.numericAnswer < 1 ||
        answer.numericAnswer > 5)
    )
      errors[q.id] = `Please select a rating for: ${q.text}`;
    if (q.type === "choice" && !q.options?.includes(clean(answer.textAnswer)))
      errors[q.id] = `Please choose one of the available options for: ${q.text}`;
    if (q.type === "ranking") {
      let ranking: unknown;
      try {
        ranking = JSON.parse(clean(answer.textAnswer));
      } catch {
        ranking = null;
      }
      if (
        !Array.isArray(ranking) ||
        ranking.length !== q.options?.length ||
        new Set(ranking).size !== ranking.length ||
        ranking.some((item) => !q.options?.includes(item))
      )
        errors[q.id] = `Please rank all of the concepts for: ${q.text}`;
    }
    if (q.type === "text" && clean(answer.textAnswer).length > 4000)
      errors[q.id] = "Please keep this response under 4,000 characters.";
  }
  for (const q of questions) {
    ...
    if (
      !answer ||
      (q.type === "text" && !clean(answer.textAnswer)) ||
      (q.type !== "text" && answer.numericAnswer == null && !clean(answer.textAnswer))
    )
      errors[q.id] ??= `Please answer: ${q.text}`;
  }
  return errors;
...
async function submit(request: Request, env: Env) {
  ...
  if (Object.keys(errors).length)
    return json({ error: "Please correct the highlighted answers.", fields: errors }, 422);
  ...
  const statements = [
    ...
    ...answers.map((a) =>
      env.DB.prepare(
        ...
        uuid,
        a.questionId,
        clean(a.prototypeKey, 80) || null,
        clean(a.layoutType, 30) || "general",
        a.answerType === "rating"
          ? "rating"
          : a.answerType === "ranking"
            ? "ranking"
            : a.numericAnswer != null
              ? "rating"
              : "text",
        Number.isInteger(a.numericAnswer) ? a.numericAnswer : null,
        clean(a.textAnswer),
      ),
    ),
  ];
...
async function admin(request: Request, env: Env) {
  ...
  if (path === "/api/admin/export") {
    ...
    const enriched = rows.results.map((row: any) => ({
      ...row,
      question_text: questionMap.get(row.question_id)?.text ?? row.question_id,
    }));
    ...
    const csv = [headers, ...enriched.map((r: any) => headers.map((h) => `"${safe(r[h])}"`))]
      .map((r) => r.join(","))
      .join("\n");
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    const url = new URL(request.url);
    if (url.pathname === "/api/survey/submit") return submit(request, env);
    if (url.pathname.startsWith("/api/admin/")) return admin(request, env);
