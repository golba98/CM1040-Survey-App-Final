...
const rating = (id: string, text: string, layoutType: LayoutType = "general"): Question => ({ id, text, type: "rating", required: true, min: 1, max: 5, layoutType });
...
const retiredFirstImpressionQuestions: Question[] = [
...
export const retiredQuestionIds = new Set(retiredFirstImpressionQuestions.map((question) => question.id));

const conceptFirstImpressionQuestions = (concept: typeof concepts[number]): Question[] => {
  const prototypeKey = `${concept.key}-bandwidth`;
  return [
    ...
    { ...choice(`${concept.key}_first_impression_purpose`, `When you first look at the ${concept.name} design, is it clear what the website is about?`, ["Yes, immediately", "Mostly", "Somewhat", "Not really", "No"]), prototypeKey },
  ...
  ].map((question) => ({ ...question, required: false, stronglyEncouraged: true }));
};

export const screenQuestions = (prototypeKey: string, conceptKey: string, conceptName: string, eraLabel: string): Question[] => [
  ...
  ...(conceptKey === "timeline" ? [
    { ...choice(`${prototypeKey}_timeline_flow`, `Are the dates and milestone cards in Timeline History ${eraLabel} easy to follow in order?`, ["Yes", "Mostly", "Somewhat", "Not really", "No"]), prototypeKey },
  ] : conceptKey === "editorial" ? [
    { ...choice(`${prototypeKey}_story_scan`, `Is it easy to scan the story sections and supporting archive content in Editorial Story ${eraLabel}?`, ["Yes", "Mostly", "Somewhat", "Not really", "No"]), prototypeKey },
  ] : [
    { ...choice(`${prototypeKey}_data_scan`, `Do the indicators, map/chart, and event records in Visual Data ${eraLabel} make the information easier to understand?`, ["Yes", "Mostly", "Somewhat", "Not really", "No"]), prototypeKey },
  ]),
].map((question) => ({ ...question, required: false, stronglyEncouraged: true }));
...
export const crossConceptQuestions: Question[] = [
  ...
  rating("navigation_clarity", "Is it clear how you would navigate to the different parts of the website?"),
  choice("navigation_destination", "Without clicking anything, can you tell what the navigation options would take you to?", ["Yes, all of them", "Most of them", "Some of them", "Very few of them", "None of them"]),
  ...
  choice("button_result", "Is it clear what would happen if you clicked the main buttons?", ["Yes", "Mostly", "Somewhat", "Not really", "No"]),
  ...
  text("readability_difficult", "Is anything difficult to read or distinguish from the background?"),
  rating("desktop_space", "How well do the desktop layouts use the available screen space?", "desktop"),
  text("desktop_scale", "Does anything on the desktop layouts feel too large or too small?", false, false),
  ...
  choice("mobile_cramped", "Does anything appear too cramped on mobile?", ["No", "A little", "Yes", "Unsure"], false),
  choice("mobile_tap_targets", "Do the buttons and controls appear large enough to comfortably tap?", ["Yes", "Mostly", "Unsure", "Not really", "No"]),
  ...
  text("mobile_information_lost", "Do you think anything important has been lost when moving from desktop to mobile?"),
  text("accessibility_barrier", "Did you notice anything that might make the website difficult for someone to use?"),
  rating("clickable_distinction", "Are clickable elements visually easy to distinguish from normal content?"),
...
export const questions: Question[] = [
  ...globalQuestions,
  ...retiredFirstImpressionQuestions,
  ...concepts.flatMap(conceptFirstImpressionQuestions),
  ...prototypes.flatMap((prototype) => screenQuestions(prototype.key, prototype.conceptKey, prototype.conceptName, prototype.eraLabel)),
